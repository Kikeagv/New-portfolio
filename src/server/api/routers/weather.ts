...
const getWeatherCondition = (code: number): { condition: string; icon: "sun" | "cloud" | "cloud-rain" | "wind" } => {
  ...
  if (code >= 71 && code <= 77) return { condition: "Snowy", icon: "cloud-rain" };
  if (code >= 80 && code <= 82) return { condition: "Rain Showers", icon: "cloud-rain" };
  if (code >= 85 && code <= 86) return { condition: "Snow Showers", icon: "cloud-rain" };
  if (code === 95) return { condition: "Thunderstorm", icon: "cloud-rain" };
  if (code >= 96 && code <= 99) return { condition: "Thunderstorm", icon: "cloud-rain" };
  ...
  getCurrent: publicProcedure
    .query(async () => {
      try {
        ...
        const { condition, icon } = getWeatherCondition(validatedData.current.weather_code);
